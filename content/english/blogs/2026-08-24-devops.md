...
[AWS Glue 6.0 is generally available](https://aws.amazon.com/blogs/aws/aws-glue-6-0-now-available-with-30-lower-price-and-full-apache-iceberg-v3-support/?utm_source=tldrdevops) in every region where Glue operates, priced 30% below prior versions with no API changes required. The runtime is modernized end-to-end: Apache Spark 4.1, Python 3.13, Scala 2.13, and full Apache Iceberg v3 via Iceberg 1.11.0.
...
[Docker moved its Verified Publisher program to a self-serve flow](https://www.docker.com/blog/docker-verified-publisher-applications-are-now-self-serve/?utm_source=tldrdevops) inside Docker Hub, removing the previous sales-team gatekeeping while keeping manual review on every application. The badge now covers all content types on Docker Hub, including MCP servers, models, sandboxes, and agents — not just container images.
...
Microsoft has published a post-incident review for a [July 23 Azure West US networking outage](https://surfingcomplexity.blog/2026/08/16/quick-thoughts-on-azure-regional-outage-from-july-23-26/?utm_source=tldrdevops) that took roughly five hours to resolve. Three things had to go wrong at once, and they did.
...
[Octopus's Easy Mode series adds a Kubernetes microservice orchestration pattern](https://octopus.com/blog/octo-easy-mode-20-microservices?utm_source=tldrdevops) for teams that have many independently deployable services and want to promote them as a unit. The pattern uses a parent orchestration project containing "Deploy a Release" steps that sequence promotion of multiple microservice projects between environments.
